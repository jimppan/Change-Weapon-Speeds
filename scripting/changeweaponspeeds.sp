...
public bool ReadWeaponSpeeds()
{
	...
	BuildPath(Path_SM, weaponSpeedsPath, sizeof(weaponSpeedsPath), "configs/weaponspeeds.ini");

	if(!FileExists(weaponSpeedsPath))
		SetFailState("[changeweaponspeeds.smx] Could not load %s", weaponSpeedsPath);

	File weaponSpeedsFile = OpenFile(weaponSpeedsPath, "rt");

	if(!weaponSpeedsFile)
		SetFailState("[superheromod.smx] Could not load %s", weaponSpeedsPath);

	char data[256];
	char szSpeed[9];
	int speed = 0;
	int weapon = 0;
	while (!IsEndOfFile(weaponSpeedsFile)) 
	{
		...
		ReadFileLine(weaponSpeedsFile, data, sizeof(data));
		TrimString(data);
